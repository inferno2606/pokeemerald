...
static const struct SearchOptionText sDexSearchTypeOptions[NUMBER_OF_MON_TYPES + 1] = // + 2 for "None" and terminator, - 1 for Mystery
{
    ...
    {gText_DexEmptyString, gTypeNames[TYPE_MONKE]},
    {gText_DexEmptyString, gTypeNames[TYPE_BIRB]},
    {gText_DexEmptyString, gTypeNames[TYPE_DINOSAUR]},
    {gText_DexEmptyString, gTypeNames[TYPE_FROG]},
    {gText_DexEmptyString, gTypeNames[TYPE_ARMORED]},
    {gText_DexEmptyString, gTypeNames[TYPE_ART]},
    {gText_DexEmptyString, gTypeNames[TYPE_ANCIENT]},
    {gText_DexEmptyString, gTypeNames[TYPE_LITTLE]},
    {gText_DexEmptyString, gTypeNames[TYPE_ANGY]},
    {gText_DexEmptyString, gTypeNames[TYPE_BABY]},
    {gText_DexEmptyString, gTypeNames[TYPE_YOKAI]},
    {gText_DexEmptyString, gTypeNames[TYPE_CHARGED]},
    ...
    {gText_DexEmptyString, gTypeNames[TYPE_TYPE]},
    {gText_DexEmptyString, gTypeNames[TYPE_GUN]},
};
...
static const struct SearchOption sSearchOptions[] =
{
    ...
    [SEARCH_TYPE_MID]   = {sDexSearchTypeOptions, 12, 13, ARRAY_COUNT(sDexSearchTypeOptions) - 1},
    [SEARCH_TYPE_RIGHT] = {sDexSearchTypeOptions, 13, 14, ARRAY_COUNT(sDexSearchTypeOptions) - 1},
...
static int DoPokedexSearch(u8 dexMode, u8 order, u8 abcGroup, u8 bodyColor, u8 type1, u8 type2, u8 type3)
{
    u16 species;
    u16 i;
    u16 resultsCount;
    ...
    // Search by type
    if (type1 != TYPE_NONE || type2 != TYPE_NONE || type3 != TYPE_NONE)
    {
        ...
        if (type2 == TYPE_NONE && type3 == TYPE_NONE)
        {
            for (i = 0, resultsCount = 0; i < sPokedexView->pokemonListCount; i++)
            {
        ...
        else if (type3 == TYPE_NONE)
        {
            for (i = 0, resultsCount = 0; i < sPokedexView->pokemonListCount; i++)
            {
                if (sPokedexView->pokedexList[i].owned)
                {
                    ...
                    types[1] = gSpeciesInfo[species].types[1];
                    types[2] = gSpeciesInfo[species].types[2];
                    if ((types[0] == type1 && (types[1] == type2 || types[2] == type2)) || (types[0] == type2 && (types[1] == type1 || types[2] == type1)))
                    {
                        sPokedexView->pokedexList[resultsCount] = sPokedexView->pokedexList[i];
                        resultsCount++;
                    }
        ...
        else
        {
            for (i = 0, resultsCount = 0; i < sPokedexView->pokemonListCount; i++)
            {
                if (sPokedexView->pokedexList[i].owned)
                {
                    ...
                    types[1] = gSpeciesInfo[species].types[1];
                    types[2] = gSpeciesInfo[species].types[2];
                    if ((types[0] == type1 && (types[1] == type2 || types[2] == type2) && (types[1] == type3 || types[2] == type3)) 
                    || (types[0] == type2 && (types[1] == type1 || types[2] == type1) && (types[1] == type3 || types[2] == type3))
                    || (types[0] == type3 && (types[1] == type1 || types[2] == type1) && (types[1] == type2 || types[2] == type2)))
                    {
                        sPokedexView->pokedexList[resultsCount] = sPokedexView->pokedexList[i];
                        resultsCount++;
                    }
...
static void Task_StartPokedexSearch(u8 taskId)
{
    ...
    u8 type2 = GetSearchModeSelection(taskId, SEARCH_TYPE_MID);
    u8 type3 = GetSearchModeSelection(taskId, SEARCH_TYPE_RIGHT);

    DoPokedexSearch(dexMode, order, abcGroup, bodyColor, type1, type2, type3);
...
#define SEARCH_BG_TYPE_SELECTION_MID    (SEARCH_TYPE_MID + SEARCH_TOPBAR_COUNT)
...
static void HighlightSelectedSearchMenuItem(u8 topBarItem, u8 menuItem)
{
    ...
    case SEARCH_TYPE_LEFT:
        ...
        break;
    case SEARCH_TYPE_MID:
        DrawSearchMenuItemBgHighlight(SEARCH_BG_TYPE_TITLE, FALSE, FALSE);
        DrawSearchMenuItemBgHighlight(SEARCH_BG_TYPE_SELECTION_MID, FALSE, FALSE);
        break;
